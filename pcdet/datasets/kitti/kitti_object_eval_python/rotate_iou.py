#####################
# CPU-only version of rotate_iou using numba JIT
...
@numba.jit(nopython=True)
def rotate_iou_kernel_eval(N, K, boxes, query_boxes, iou, criterion=-1):
    for i in range(N):
        for j in range(K):
            iou[i * K + j] = devRotateIoUEval(query_boxes[j * 5:j * 5 + 5],
                                               boxes[i * 5:i * 5 + 5], criterion)


def rotate_iou_gpu_eval(boxes, query_boxes, criterion=-1, device_id=0):
    """rotated box iou running on CPU with numba JIT.

    Args:
        boxes (np.ndarray: [N, 5]): rbboxes. format: centers, dims, angles
        query_boxes (np.ndarray: [K, 5]): query rbboxes.
        criterion: -1 for IoU, 0 for query area, 1 for box area
        device_id: unused, kept for API compatibility

    Returns:
        iou (np.ndarray: [N, K]): IoU matrix
    """
    ...
    if N == 0 or K == 0:
        return iou
    rotate_iou_kernel_eval(N, K, boxes.reshape(-1), query_boxes.reshape(-1),
